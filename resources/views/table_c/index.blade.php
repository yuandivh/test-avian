                ...
                <table class="w-full text-md text-center table-fixed max-w-7xl">
                    <thead class="sticky top-0 bg-gray-200 border-b border-gray-300">
                        <tr>
                            ...
                            <th class="w-1/3 p-3">Area sales</th>
                            <th class="w-1/3 p-3">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($tablec as $data )
                        <tr class="border-b border-gray-400">
                            <td class="p-3">{{ $data->kode_toko }}</td>
                            <td class="p-3">{{ $data->area_sales }}</td>
                            <td class="p-3">
                                <div class="flex gap-5 justify-evenly items-center">
                                    <a href="{{ route('table_c.edit',['tablecId'=>$data->id]) }}">
                                    ...
                                    <form action="{{ route('table_c.destroy',['tablecId'=>$data->id]) }}" method="post" class="delete-form">
                                        @csrf
                                        @method('delete')
            ...
            <form action="{{ route('table_c.import') }}" method="post" enctype="multipart/form-data">
                ...
                <input
                    ...
                    <div class="flex justify-end gap-3">
                        <a href="{{ route('table_c.export-template') }}"
