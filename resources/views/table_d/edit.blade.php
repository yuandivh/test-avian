    ...
    <div class="flex  justify-center items-center min-h-screen w-full">
        <div class="w-full max-w-md">
            ...
            <form action="{{ route('table_a.update',['tableaId' => $tablea->id ]) }}" method="post" class="space-y-4 mb-4">
                @csrf
                @method('put')
                <input value="{{ $tablea->kode_toko_baru ?? old('kode_toko_baru') }}" type="number" name="kode_toko_baru" id="" placeholder="Kode toko baru" min="0"
                ...
                @enderror
                <input value="{{ $tablea->kode_toko_lama ?? old('kode_toko_lama') }}" type="number" name="kode_toko_lama" id="" placeholder="Kode toko lama" min="0"
                class="border-2 w-full px-2 py-1 rounded-md
